...
echo "=============================="
echo " Starting diff size ratio experiments"
echo " TAG=${TAG}"
...
for SCALE in "${SCALE_FACTORS[@]}"; do
    for i in "${!ENTRY_SIZES[@]}"; do
        ...
        mkdir -p RocksDB SuccinctKV RangeReduce[lb=T^-1ANDre=1] # RangeReduce[lb=0] RangeReduce[lb=T^-1] RangeReduce[lb=0ANDsmlck=0]

        # --------------------------------------------------------
        # Workload Generation
        # --------------------------------------------------------
        # echo "Generating specs for Tectonic..."
        # python3 ../../scripts/generate_specs.py -I ${INSERTS} -U ${UPDATES} -S ${RANGE_QUERIES} -Y ${SELECTIVITY} -E ${ENTRY_SIZE} -L ${LAMBDA}
        ...
        # --------------------------------------------------------
        for target in "../SuccinctKV" "../RangeReduce[lb=T^-1ANDre=1]"; do
...
HOSTNAME=$(hostname)
MESSAGE="Size Ratio ${SIZE_RATIO} Experiments Completed on ${HOSTNAME} (TAG=${TAG})"
