...
void DBImpl::ApplyRangeQueryEdits() {
  for (auto deleted_file : edits_->GetDeletedFiles()) {
    std::cout << "Level: " << deleted_file.first
  ...
  versions_->LogAndApply(cfd, cf_opts, read_options, &(*edits_), &mutex_,
...

  {
    ...
    TableBuilderOptions tboptions(
        *cfd_->ioptions(), *mutable_cf_options_, cfd_->internal_comparator(),
        ...
        db_id, db_session_id, 0 /* target_file_size */, meta_.fd.GetNumber());
...
Status DBImpl::WriteLevelNFile() {
...

  {
    ...
    event_logger_.Log() << "job" << job_context.job_id << "event"
                        ...
                        << "file_number"
                        << range_query_memtable_->GetFileNumber()
                        << "num_entries" << range_query_memtable_->num_entries()
                        << "num_deletes" << range_query_memtable_->num_deletes()
                        << "total_data_size"
  ...
  // TODO: (shubham)
  // Flush the table to level N take reference from Flush_Job::WriteLevel0Table
...
Status DBImpl::FlushPartialOrRangeFile(
  ...
  // TODO: (shubham) complete this function
}
...
Status DBImpl::FlushPartialOrRangeFiles(
  ...
  if (flush_reason == FlushReason::kPartialFlush) {
    ...
    s = FlushPartialOrRangeFile(cfd, mutable_cf_options_copy, made_progress,
                                job_context, flush_reason, superversion_context,
  ...
  *reason = FlushReason::kOthers;  // Why this is updated ?
  ...
  while (!flush_queue_.empty()) {
    ...
    for (const auto& iter : flush_req.cfd_to_max_mem_id_to_persist) {
      ...
      // if it is kPartialFlush and just_delete is true
      // add this file to the edits and we are done!
      if (flush_reason == FlushReason::kPartialFlush && just_delete) {
        uint64_t file_number = cfd->current()
                                   ->storage_info()
                                   ->LevelFilesBrief(level)
                                   .files[file_index]
                                   .file_metadata->fd.GetNumber();
        edits_->DeleteFile(level, file_number);
        continue;
      }
...
void DBImpl::AddPartialOrRangeFileFlushRequest(FlushReason flush_reason,
  ...
  FlushRequest req{flush_reason, {{cfd, 0}}, file_index, level, just_delete};
