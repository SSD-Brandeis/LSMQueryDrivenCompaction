  ...
  while (!workload_file.eof()) {
    ...
    case 'S':  // scan: range query
      workload_file >> start_key >> end_key;
      {
        ...
        uint64_t skipped_entries_count = 0;
        uint64_t compacted_entries_count = 0;
        uint64_t returned_entries_count = 0;
        ...
        it->Refresh(start_key, end_key,
          read_entries_count, env->enable_range_query_compaction);
#ifdef TIMER
        ...
        auto total_entries_read = read_options.range_query_options->count_of_entries;
#endif  // TIMER
        it->Reset(skipped_entries_count, compacted_entries_count);
#ifdef TIMER
        ...
        range_queries_.emplace_back(std::make_tuple(
          duration.count(),
          partial_range_flush_listener->useful_data_blocks_size_,
          partial_range_flush_listener->useful_file_size_,
          partial_range_flush_listener->useful_entries_,
          total_entries_read,
          partial_range_flush_listener->un_useful_data_blocks_size_,
          partial_range_flush_listener->un_useful_file_size_,
          partial_range_flush_listener->un_useful_entries_,
          skipped_entries_count, compacted_entries_count,
          refresh_duration,
