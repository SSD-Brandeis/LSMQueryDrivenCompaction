...
void configOptions(EmuEnv *_env, Options *op, BlockBasedTableOptions *t_op,
  ...
  op->max_write_buffer_number =
      _env->max_write_buffer_number;  // min 2 // !YBS-sep07-XX!
...
bool FlushMemTableMayAllComplete(DB *db) {
    ...
    bool success = db->GetIntProperty("rocksdb.mem-table-flush-pending",
&pending_flush)
                              &&
db->GetIntProperty("rocksdb.num-running-flushes", &running_flush); while
(pending_flush || running_flush || !success) { sleep_for_ms(200); success =
...
static_cast_with_check<ColumnFamilyHandleImpl>(db->DefaultColumnFamily())->cfd()).ok();
} */
  ...
  while (!workload_file.eof()) {
    ...
    switch (instruction) {
      ...
      case 'D':  // point delete
        ...
        //   start_key << " " << end_key << "" << std::endl; s =
        //   db->DeleteRange(w_options, ToString(start_key), ToString(end_key));
        //   assert(s.ok());
...
int parse_arguments2(int argc, char *argv[], EmuEnv *_env) {
  ...
  */
  ...
  _env->delete_persistence_latency =
      del_per_th_cmd ? args::get(del_per_th_cmd) : 0;  // !YBS-feb15-XXI!
