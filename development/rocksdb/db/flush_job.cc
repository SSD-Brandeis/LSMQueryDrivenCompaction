...
Status PartialOrRangeFlushJob::Run(LogsWithPrepTracker* /*prep_tracker*/,
  ...
  // NOTE: again not sure about the mempurge thing here.
  ...
  TEST_SYNC_POINT("PartialOrRangeFlushJob::End");
  return s;
}
...
Status PartialOrRangeFlushJob::WriteLevelNTable() {
  AutoThreadOperationStageUpdater stage_updater(
      ThreadStatus::STAGE_FLUSH_WRITE_LN);
  db_mutex_->AssertHeld();
  const uint64_t start_micros = clock_->NowMicros();
  const uint64_t start_cpu_micros = clock_->CPUMicros();
  Status s;
...

  {
    ...
    total_num_deletes = memtable_->num_deletes();
    total_data_size = memtable_->get_data_size();
    total_memory_usage = memtable_->ApproximateMemoryUsage();
...

    {
      ...
      const uint64_t current_time = static_cast<uint64_t>(_current_time);
      uint64_t oldest_key_time = memtable_->ApproximateOldestKeyTime();
      uint64_t oldest_ancester_time = std::min(current_time, oldest_key_time);
...
Status PartialOrRangeFlushJob::WritePartialTable() {
  AutoThreadOperationStageUpdater stage_updater(
