...
echo "=============================="
echo " Starting different file sizes experiments"
echo " TAG=${TAG}"
echo "=============================="
...
for SCALE in "${SCALE_FACTORS[@]}"; do
    for i in "${!ENTRY_SIZES[@]}"; do
        ...
        EXP_DIR="experiments-${TAG}-${SCALE}x-U${UPDATES}-E${entry_size}-B${entries_per_page}-P${PAGES_PER_FILE}-S${RANGE_QUERIES}-Y${SELECTIVITY}-T${SIZE_RATIO}"
        ...
        mkdir -p RocksDB SuccinctKV RangeReduce[lb=T^-1] RangeReduce[lb=T^-1ANDre=1]
        ...
        # --------------------------------------------------------
        for target in "../SuccinctKV" "../RangeReduce[lb=T^-1]" "../RangeReduce[lb=T^-1ANDre=1]"; do
